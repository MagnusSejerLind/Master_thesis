...
cf_nl = 0.5;    % coeffcient of nonlinear damping force
kf_nl = 0.5;    % coeffcient of nonlinear stiffness force
...
[H] = ToeplitzMatrix(N,ms,r,Ad,Bd,Cd,Dd);  % (H tilde)
...
[alpha,beta] = raylieghDamp(omegaN,xi_int);
C_con = alpha*M + beta*K;
C_modal_con = round(Phi'*C*Phi,10); % Damping matrix
xi = diag(C_modal) ./ (2*omegaN);   
